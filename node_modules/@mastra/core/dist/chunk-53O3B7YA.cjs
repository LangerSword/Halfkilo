...
function toScorerTargetEntityType(entityType) {
  switch (entityType) {
    case "AGENT":
      return chunkL56725IW_cjs.EntityType.AGENT;
    case "WORKFLOW":
      return chunkL56725IW_cjs.EntityType.WORKFLOW_RUN;
    default:
      return void 0;
  }
}
...
async function validateAndSaveScore(storage, payload) {
  ...
  const payloadToSave = chunkP5C4HEAB_cjs.saveScorePayloadSchema.parse(payload);
...
function isWorkflow(target) {
  return target instanceof chunkZYCSOEMD_cjs.Workflow;
}
...
async function executeWorkflow(target, item, targetOptions) {
  const observabilityContext = chunk4FFOOHE5_cjs.resolveObservabilityContext(item);
  ...
  return {
    ...
    entityType: chunkL56725IW_cjs.EntityType.WORKFLOW_RUN,
    scoringData: {
      input: item.input,
...
async function executeAgent(agent, item, targetOptions) {
  ...
  if (chunkZYCSOEMD_cjs.isSupportedLanguageModel(model)) {
    ...
    return {
      ...result,
      entityType: chunkL56725IW_cjs.EntityType.AGENT
    };
  } else {
...
async function extractTrajectoryFromTraceStore(storage, traceId, spanId) {
  ...
  try {
    ...
    return chunkP5C4HEAB_cjs.extractTrajectoryFromTrace(trace.spans, spanId);
  } catch {
    return void 0;
  }
}
async function runScorers(scorers, targetResult, item, storage) {
  ...
  if (Array.isArray(scorers)) {
    ...
    if (scorers.trajectory) {
      ...
      const trajectory = traceTrajectory ?? (rawOutput ? chunkP5C4HEAB_cjs.extractTrajectory(rawOutput) : { steps: [] });
          ...
          for (const scorer of stepScorers) {
            try {
              const score = await scorer.run({
                ...
                scoreSource: "experiment",
                targetScope: "span",
                targetEntityType: chunkL56725IW_cjs.EntityType.WORKFLOW_STEP,
                targetTraceId
              });
    ...
    if (scorers.trajectory) {
      ...
      if (!trajectory) {
        ...
        trajectory = stepResults ? chunkP5C4HEAB_cjs.extractWorkflowTrajectory(stepResults, stepExecutionPath) : { steps: [] };
      }
      for (const scorer of scorers.trajectory) {
        try {
          const score = await scorer.run({
            ...
            scoreSource: "experiment",
            targetScope: "trajectory",
            targetEntityType: chunkL56725IW_cjs.EntityType.TRAJECTORY,
            targetTraceId,
            targetSpanId: targetResult.spanId
          });
          trajectoryScorerResults[scorer.id] = score;
...
//# sourceMappingURL=chunk-53O3B7YA.cjs.map
//# sourceMappingURL=chunk-53O3B7YA.cjs.map