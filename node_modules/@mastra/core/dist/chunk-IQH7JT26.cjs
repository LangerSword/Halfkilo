...
var chunkZYCSOEMD_cjs = require('./chunk-ZYCSOEMD.cjs');
...
var StepExecutor = class extends chunkFCQNDFEW_cjs.MastraBase {
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkZYCSOEMD_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await chunkQFGYBEP3_cjs.executeWithContext({
        span: params.tracingContext?.currentSpan,
        fn: () => step.execute(
          chunkZYCSOEMD_cjs.createDeprecationProxy(
            {
              ...
              getStepResult: chunkZYCSOEMD_cjs.getStepResult.bind(this, stepResults),
              suspend: async (suspendPayload, suspendOptions) => {
                const { suspendData, validationError: validationError2 } = await chunkZYCSOEMD_cjs.validateStepSuspendData({
              ...
              ...chunk4FFOOHE5_cjs.createObservabilityContext(params.tracingContext)
            },
            {
              paramName: "runCount",
              deprecationMessage: chunkZYCSOEMD_cjs.runCountDeprecationMessage,
              logger: this.logger
            }
          )
        )
      ...
      return {
        ...
        tripwire: error instanceof chunkZYCSOEMD_cjs.TripWire ? {
      ...
      chunkZYCSOEMD_cjs.createDeprecationProxy(
        {
          ...
          abortSignal: abortController?.signal,
          // TODO
          ...chunk4FFOOHE5_cjs.createObservabilityContext(),
          iterationCount
        },
        {
          paramName: "runCount",
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkZYCSOEMD_cjs.TripWire(
  ...
  if (resumeData !== void 0 && forEachIndex === void 0 && currentResult?.output?.length > 0) {
    ...
    if (suspendedIndices.length > 0) {
      ...
      for (const suspIdx of indicesToResume) {
        updatedOutput[suspIdx] = chunkSRQR5UN6_cjs.createPendingMarker();
      }
  ...
  async processWorkflowStepRun({
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkZYCSOEMD_cjs.createTimeTravelExecutionParams({
    ...
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkZYCSOEMD_cjs.validateStepResumeData({
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkZYCSOEMD_cjs.Workflow({
    id: opts.id,
...
function isAgent(input) {
  return input instanceof chunkZYCSOEMD_cjs.Agent;
}
function isToolStep(input) {
  return input instanceof chunkLDLHW3QN_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkZYCSOEMD_cjs.Agent) && !(input instanceof chunkLDLHW3QN_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkZYCSOEMD_cjs.Agent) && !(obj instanceof chunkLDLHW3QN_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
async function processAgentStream(params) {
  ...
  for await (const chunk of fullStream) {
    ...
    if (streamFormat !== "legacy") {
      await chunkZYCSOEMD_cjs.forwardAgentStreamChunk({ writer, chunk });
    }
  }
  try {
    await pubsub.publish(`workflow.events.v2.${runId}`, {
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const observabilityContext = chunk4FFOOHE5_cjs.resolveObservabilityContext(obsFields);
      const logger = mastra?.getLogger();
      const toolData = {
        name: params.name,
        args: inputData
      ...
      const isV2Model = chunkZYCSOEMD_cjs.isSupportedLanguageModel(modelInfo);
...
function createStepFromProcessor(processor) {
  const getProcessorEntityType = (phase) => {
    switch (phase) {
      case "input":
        return chunkL56725IW_cjs.EntityType.INPUT_PROCESSOR;
      case "inputStep":
        return chunkL56725IW_cjs.EntityType.INPUT_STEP_PROCESSOR;
      case "outputStream":
      case "outputResult":
        return chunkL56725IW_cjs.EntityType.OUTPUT_PROCESSOR;
      case "outputStep":
        return chunkL56725IW_cjs.EntityType.OUTPUT_STEP_PROCESSOR;
      default:
        return chunkL56725IW_cjs.EntityType.OUTPUT_PROCESSOR;
    }
  };
  ...
  return {
    ...
    inputSchema: chunkXB4FLS7A_cjs.toStandardSchema(chunkZYCSOEMD_cjs.ProcessorStepSchema),
    outputSchema: chunkXB4FLS7A_cjs.toStandardSchema(chunkZYCSOEMD_cjs.ProcessorStepOutputSchema),
    execute: async ({ inputData, requestContext, ...obsFields }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkZYCSOEMD_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      const processorObservabilityContext = chunk4FFOOHE5_cjs.createObservabilityContext(
      ...
      if (processorStates) {
        ...
        if (!ps) {
          ps = new chunkZYCSOEMD_cjs.ProcessorState();
      ...
      const passThrough = {
        ...
        messageList: messageList ?? (Array.isArray(messages) ? new chunkRJAOQKHJ_cjs.MessageList().add(messages, "input").addSystem(systemMessages ?? []) : void 0),
        stepNumber,
        systemMessages,
        streamParts,
        state,
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          const result = await chunkQFGYBEP3_cjs.executeWithContext({ span: processorSpan, fn });
          processorSpan?.end({ output: result });
          return result;
        } catch (error) {
          if (error instanceof chunkZYCSOEMD_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunkRJAOQKHJ_cjs.MessageList) {
                ...
                chunkZYCSOEMD_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkZYCSOEMD_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
            ...
            if (processor.processOutputStream && part) {
              ...
              if (!processorSpan2 && parentSpan) {
                processorSpan2 = parentSpan.createChildSpan({
                  ...
                  entityType: chunkL56725IW_cjs.EntityType.OUTPUT_PROCESSOR,
                  entityId: processor.id,
                  entityName: processor.name ?? processor.id,
              ...
              const processorObservabilityContext2 = chunk4FFOOHE5_cjs.createObservabilityContext(
...
var EventedWorkflow = class extends chunkZYCSOEMD_cjs.Workflow {
  constructor(params) {
    super(params);
...
var EventedRun = class extends chunkZYCSOEMD_cjs.Run {
  constructor(params) {
    super(params);
  ...
  stream({
    ...
    this.streamOutput = new chunkZYCSOEMD_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
...
var EventedExecutionEngine = class extends chunkZYCSOEMD_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkZYCSOEMD_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      cleanStepResults[stepId] = chunkZYCSOEMD_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
//# sourceMappingURL=chunk-IQH7JT26.cjs.map
//# sourceMappingURL=chunk-IQH7JT26.cjs.map