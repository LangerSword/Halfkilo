...
var chunkIQH7JT26_cjs = require('./chunk-IQH7JT26.cjs');
var chunkZMGEKD6A_cjs = require('./chunk-ZMGEKD6A.cjs');
var chunkZPHI6QAA_cjs = require('./chunk-ZPHI6QAA.cjs');
var chunkTSSVO2DE_cjs = require('./chunk-TSSVO2DE.cjs');
var chunk53O3B7YA_cjs = require('./chunk-53O3B7YA.cjs');
...
var chunk5POWUC5M_cjs = require('./chunk-5POWUC5M.cjs');
...
var Mastra = class {
  ...
  get datasets() {
    if (!this.#datasets) {
      this.#datasets = new chunkTSSVO2DE_cjs.DatasetsManager(this);
    }
  ...
  registerExporter(exporter, instance, entrypoint) {
    if (this.#observability instanceof chunk4FFOOHE5_cjs.NoOpObservability) {
   ...
   */
  constructor(config) {
    chunkZPHI6QAA_cjs.initContextStorage();
    this.#serverCache = new chunkI4YMMVQ3_cjs.InMemoryServerCache();
    ...
    const workflowEventProcessor = new chunkIQH7JT26_cjs.WorkflowEventProcessor({ mastra: this });
    ...
    if (config?.logger === false) {
      logger = chunkFLMCO42Y_cjs.noopLogger;
    } else {
      if (config?.logger) {
    ...
    if (storage) {
      storage = chunkZYCSOEMD_cjs.augmentWithInit(storage);
    }
    if (config?.observability) {
      if (typeof config.observability.getDefaultInstance === "function") {
        ...
        this.#observability = new chunk4FFOOHE5_cjs.NoOpObservability();
      }
    } else {
      this.#observability = new chunk4FFOOHE5_cjs.NoOpObservability();
    }
    const dualLogger = new chunkFLMCO42Y_cjs.DualLogger(this.#logger, () => this.loggerVNext);
    ...
    for (const gateway of chunk5POWUC5M_cjs.defaultGateways) {
    ...
    chunkMR7ZWBL6_cjs.registerHook("onScorerRun" /* ON_SCORER_RUN */, chunk53O3B7YA_cjs.createOnScorerHook(this));
    this.#observability.setMastraContext({ mastra: this });
  ...
  addAgent(agent, key, options) {
    ...
    let mastraAgent;
    if (chunkZMGEKD6A_cjs.isToolLoopAgentLike(agent)) {
      mastraAgent = chunkZMGEKD6A_cjs.toolLoopAgentToMastraAgent(agent, { fallbackName: key });
    } else {
      mastraAgent = agent;
    }
   ...
   */
  setStorage(storage) {
    this.#storage = chunkZYCSOEMD_cjs.augmentWithInit(storage);
  }
  setLogger({ logger }) {
   ...
   */
  get loggerVNext() {
    return this.#observability.getDefaultInstance()?.getLoggerContext?.() ?? chunk4FFOOHE5_cjs.noOpLoggerContext;
  }
  /**
   ...
   */
  get metrics() {
    return this.#observability.getDefaultInstance()?.getMetricsContext?.() ?? chunk4FFOOHE5_cjs.noOpMetricsContext;
  }
  getServerMiddleware() {
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-7SHON2PJ.cjs').then(async ({ GatewayRegistry }) => {
...
//# sourceMappingURL=chunk-LILBXY3Z.cjs.map
//# sourceMappingURL=chunk-LILBXY3Z.cjs.map