...
var AIV5Adapter = class {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkY3APFUXZ_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkY3APFUXZ_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
...
var AIV6Adapter = class _AIV6Adapter {
  static toUIMessage(dbMsg) {
    ...
    if (!hasToolInvocationParts || !hasReasoningParts || !hasFileParts || !hasTextParts) {
      for (const part of v5Message.parts) {
        if (chunkY3APFUXZ_cjs.isToolUIPart(part)) {
  ...
  static fromUIMessage(uiMsg) {
    const compatibleParts = uiMsg.parts.filter((part) => {
      ...
      if (chunkMMHYKJDT_cjs.isToolUIPart(part) && isV6OnlyToolState(part.state)) return false;
      return true;
    });
    ...
    for (const part of uiMsg.parts) {
      ...
      if (!chunkMMHYKJDT_cjs.isToolUIPart(part) || !isV6OnlyToolState(part.state)) {
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunkY3APFUXZ_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  const msgs = messages.map((m) => {
    ...
    const safeParts = m.parts.filter((p) => {
      ...
      if (!chunkY3APFUXZ_cjs.isToolUIPart(p)) return true;
    ...
    const sanitized = {
      ...
      parts: mergedParts.map((part) => {
        ...
        if (chunkY3APFUXZ_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkY3APFUXZ_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkY3APFUXZ_cjs.isToolUIPart(nextPart)) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
      if (nextPart && chunkY3APFUXZ_cjs.isToolUIPart(nextPart) && !part.providerExecuted && nextPart.providerExecuted && (nextPart.state === "output-available" || nextPart.state === "output-error")) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
    }
  }
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkSQINVD7G_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
...
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  const result = restoreAssistantFileProviderMetadata(chunkY3APFUXZ_cjs.convertToModelMessages(preprocessed), preprocessed);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkY3APFUXZ_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
//# sourceMappingURL=chunk-RJAOQKHJ.cjs.map
//# sourceMappingURL=chunk-RJAOQKHJ.cjs.map