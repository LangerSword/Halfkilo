...
async function executeAgent(agent, item, signal, requestContext) {
  ...
  const rawResult = chunkZYCSOEMD_cjs.isSupportedLanguageModel(model) ? await agent.generate(input, {
    scorers: {},
...
function toScorerTargetEntityType(targetType) {
  switch (targetType) {
    ...
    case "scorer":
      return chunkL56725IW_cjs.EntityType.SCORER;
    default:
      return void 0;
  }
}
  ...
  const settled = await Promise.allSettled(
    scorers.map(async (scorer) => {
      ...
      if (storage && result.score !== null) {
        try {
          await chunk53O3B7YA_cjs.validateAndSaveScore(storage, {
            scorerId: scorer.id,
            score: result.score,
...
async function runScorerSafe(scorer, item, output, scorerInput, scorerOutput, targetType, targetTraceId) {
  try {
...
//# sourceMappingURL=chunk-TSSVO2DE.cjs.map
//# sourceMappingURL=chunk-TSSVO2DE.cjs.map