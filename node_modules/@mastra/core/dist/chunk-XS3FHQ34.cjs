...
var CoreToolBuilder = class extends chunkFCQNDFEW_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkLDLHW3QN_cjs.isVercelTool(this.originalTool) && !chunkLDLHW3QN_cjs.isProviderDefinedTool(this.originalTool) && (input.autoResumeSuspendedTools || this.originalTool.id?.startsWith("agent-") || this.originalTool.id?.startsWith("workflow-"))) {
  ...
  getParameters = () => {
    if (chunkLDLHW3QN_cjs.isVercelTool(this.originalTool)) {
    ...
    if ("type" in tool && (tool.type === "provider-defined" || tool.type === "provider") && "id" in tool && typeof tool.id === "string" && tool.id.includes(".")) {
      ...
      if (parameters !== void 0 && parameters !== null) {
        if (typeof parameters === "object" && "jsonSchema" in parameters) {
          ...
          const jsonSchema2 = chunkXB4FLS7A_cjs.standardSchemaToJSONSchema(parameters, { io: "output" });
  ...
  createExecute(tool, options, logType) {
    ...
    const mcpMeta = !chunkLDLHW3QN_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
    const execFunction = async (args, execOptions, toolSpan) => {
      try {
        let result;
        let suspendData = null;
        if (chunkLDLHW3QN_cjs.isVercelTool(tool)) {
          result = await chunkQFGYBEP3_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunk4FFOOHE5_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            runId: options.runId,
            requestContext: execOptions.requestContext ?? options.requestContext ?? new chunk5WBEMKE2_cjs.RequestContext(),
            ...
            ...chunk4FFOOHE5_cjs.createObservabilityContext({ currentSpan: toolSpan }),
          ...
          if (resumeData) {
            const resumeValidation = chunkLDLHW3QN_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunkQFGYBEP3_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunkLDLHW3QN_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkLDLHW3QN_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkLDLHW3QN_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      const toolSpan = chunkQFGYBEP3_cjs.getOrCreateSpan({
        ...
        input: args,
        entityType: chunkL56725IW_cjs.EntityType.TOOL,
      ...
      try {
        ...
        const { data, error } = chunkLDLHW3QN_cjs.validateToolInput(parameters, args, options.name);
  ...
  build() {
    ...
    if (chunkXB4FLS7A_cjs.isStandardSchemaWithJSON(originalSchema)) {
      const inputJsonSchema = applicableLayer ? applicableLayer.toJSONSchema(originalSchema) : chunkXB4FLS7A_cjs.standardSchemaToJSONSchema(originalSchema, { io: "input" });
      processedInputSchema = schemaCompat.jsonSchema(inputJsonSchema);
    } else {
      if (originalSchema) {
    ...
    let needsApprovalFn;
    if (chunkLDLHW3QN_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkLDLHW3QN_cjs.Tool) && !chunkLDLHW3QN_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-XS3FHQ34.cjs.map
//# sourceMappingURL=chunk-XS3FHQ34.cjs.map