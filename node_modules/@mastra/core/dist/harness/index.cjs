...
var chunkLILBXY3Z_cjs = require('../chunk-LILBXY3Z.cjs');
...
var planCounter = 0;
var askUserTool = chunkLDLHW3QN_cjs.createTool({
  id: "ask_user",
...
var submitPlanTool = chunkLDLHW3QN_cjs.createTool({
  id: "submit_plan",
...
var taskWriteTool = chunkLDLHW3QN_cjs.createTool({
  id: "task_write",
...
var taskCheckTool = chunkLDLHW3QN_cjs.createTool({
  id: "task_check",
...
function createSubagentTool(opts) {
  ...
  return chunkLDLHW3QN_cjs.createTool({
    id: "subagent",
...
- The task is self-contained and can be delegated`,
    inputSchema: v4.z.object({
      ...
      modelId: v4.z.string().optional().describe("Optional model ID override for this task.")
    }),
    execute: async ({ agentType, task, modelId }, context) => {
      ...
      const subagent = new chunkZYCSOEMD_cjs.Agent({
      ...
      const allWorkspaceToolNames = workspace && allowedWs ? new Set(
        Object.keys(
          await chunkRNJ2FXGD_cjs.createWorkspaceTools(workspace, {
...
var Harness = class {
  ...
  constructor(config) {
    ...
    if (config.workspace instanceof chunkRNJ2FXGD_cjs.Workspace) {
  ...
  async init() {
    if (this.config.storage) {
      this.#internalMastra = new chunkLILBXY3Z_cjs.Mastra({ logger: false, storage: this.config.storage });
      await this.#internalMastra.getStorage().init();
    }
    if (this.config.workspace && !this.workspaceInitialized && !this.workspaceFn) {
      try {
        if (!this.workspace) {
          this.workspace = new chunkRNJ2FXGD_cjs.Workspace(this.config.workspace);
        }
  ...
  async getCurrentModelAuthStatus() {
    ...
    try {
      const { PROVIDER_REGISTRY } = await import('../provider-registry-7SHON2PJ.cjs');
  ...
  applyDisplayStateUpdate(event) {
    ...
    switch (event.type) {
      ...
      case "tool_update": {
        ...
        if (tool) {
          tool.partialResult = typeof event.partialResult === "string" ? event.partialResult : chunkXS3FHQ34_cjs.safeStringify(event.partialResult);
        }
        break;
      }
      case "tool_end": {
