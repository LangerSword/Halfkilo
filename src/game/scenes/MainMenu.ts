...
export class MainMenu extends Scene {
    ...
    create() {
        ...
        this.createButton(centerX, startY + buttonSpacing * 3, '⚔ Battle Arena ⚔', () => {
    ...
    updateButtonStyle(button: Phaser.GameObjects.Graphics, shadow: Phaser.GameObjects.Graphics, x: number, y: number, width: number, height: number, radius: number, isHover: boolean) {
        button.clear();
        shadow.clear();
    ...
    showInstructions() {
        ...
        const closeElements = this.addCloseButton(centerX, centerY + 79, () => {
            this.destroyInstructionElements({ ...elements, ...instructionContent, closeButton: closeElements.button, closeText: closeElements.text });
        });
    ...
    createInstructionPanel(centerX: number, centerY: number) {
    ...
    addInstructionContent(centerX: number, centerY: number) {
    ...
    addCloseButton(x: number, y: number, callback: () => void) {
    ...
    destroyInstructionElements(elements: any) {
        elements.overlay.destroy();
        elements.panel.destroy();
        elements.title.destroy();

        elements.textElements.forEach((text: Phaser.GameObjects.Text) => text.destroy());
